...
def wkt_to_graph(wkt_list,pathImage):
    ...
    n_threads = 12
    verbose = False
    im_file = '../ground_truth/SN3_roads_train_AOI_2_Vegas_PS-MS_img2.tif'
    pickle_protocol = 4
    simplify_graph = True
    ...
    if simplify_graph:
        ...
        if pathImage == '../ground_truth/SN3_roads_train_AOI_2_Vegas_PS-MS_img2.tif':
            out_file = "./Ground_Truth.gpickle"
            # save
            if len(Gout.nodes()) == 0:
                nx.write_gpickle(Gout, out_file, protocol=pickle_protocol)
                return
            nx.write_gpickle(Gout, out_file, protocol=pickle_protocol)
            print('The file: ' + out_file + ' created successful ')
        else:
            out_file = "./Proposal.gpickle"
            # save
            if len(Gout.nodes()) == 0:
                nx.write_gpickle(Gout, out_file, protocol=pickle_protocol)
                return
